...
static float box_intersection(const facelandmark_box_t *a, const facelandmark_box_t *b) {
...
static float box_union(const facelandmark_box_t *a, const facelandmark_box_t *b) {
...
static float box_iou(const facelandmark_box_t *a, const facelandmark_box_t *b) {
...
static void forward_region_layer(facelandmark_region_layer_t *rl, facelandmark_box_info_t *bx) {
    float probs[2];
    float *box;
    const float *pred_box, *landm, *anc;
    for (uint32_t i= 0; i < rl->anchor_num; i++) {
        facelandmark_softmax(&rl->clses_input[i * 2], 2, probs);
...
static void do_nms_sort(const facelandmark_box_info_t *bx) {
    ...
    for (uint32_t i= 0; i < bx->cls_num; i++) {
        ...
        for (uint32_t k= 0; k < bx->row_idx; k++) {
            ...
            facelandmark_box_t *a= (facelandmark_box_t *)&box[(sort_idx[k].idx / bx->box_len) * bx->box_len];

            for (uint32_t q= k + 1; q < bx->row_idx; q++) {
                facelandmark_box_t *b= (facelandmark_box_t *)&box[(sort_idx[q].idx / bx->box_len) * bx->box_len];
